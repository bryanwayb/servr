...
function HttpServer(instance, config, useHttps) {
    ...
    for(var i = 0; i < this._hosts.length; i++) {
        ...
        var contextEntry = {
            ...
            extensions: {
                request: [ ], // When a request begins processing. Should return a response object or false, undefined, or null to discontinue processing
        ...
        if(!contextEntry.config.directories['/']) { // '/' is required
            logger.log(enums.LogType.Error, 'No root path given for the required path \'/\'');
        }
            ...
            if(contextEntry.config.directories.hasOwnProperty(urlPath)) {
                ...
                try {
                    if(!fs.statSync(resolvedPath).isDirectory()) { // TODO: Allow lstat and isSymbolicLink() with options
                        logger.log(enums.LogType.Warning, 'Path \'' + resolvedPath + '\' is not a directory');
                        deleteFromDirList = true;
                    }
                }
                catch(error) {
                    logger.log(enums.LogType.Warning, 'Directory \'' + resolvedPath + '\' does not exist');
                    deleteFromDirList = true;
                }
        ...
        for(var extensionName in contextEntry.config.extensions) {
            if(contextEntry.config.extensions.hasOwnProperty(extensionName)) {
                try {
                    ...
                    logger.log(enums.LogType.Fatal, 'Error loading extension ' + extensionName, ex);
                }
            }
        }
...
HttpServer.prototype.request = function(request, response) {
    ...
    for(i = 0; i < len; i++) {
        ...
        if(!(response = requestInit.call(this, requestInit.config, request, response))) {
            return;
        }
    }
