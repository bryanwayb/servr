...
function HttpServer(instance, config, useHttps) {
    ...
    // Load extensions
    this._extensions = {
        request: [ ], // When a request begins processing
...
HttpServer.prototype.request = function(request, response) {
    var i, len = this._extensions.request.length;
    for(i = 0; i < len; i++) {
        var requestInit = this._extensions.request[i];
        requestInit.call(this, requestInit.config, request, response);
    }
    ...
    len = this._extensions.url.length;
    for(i = 0; i < len; i++) {
    ...
    var physicalPath = path.join(this._rootPathMap[urlMapIndex].path, filterUrl.slice(matchLengthMax));
