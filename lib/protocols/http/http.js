...
HttpServer.prototype.request = function(request, response) {
    ...
    for(i = 0; i < len; i++) {
        ...
        var newResponse;
        if(!(newResponse = requestInit.call(this, requestInit.config, request, response))) {
            this._errorPage(500, request, response); // Unable to create the new response object in the extension
            return;
        }
        else {
