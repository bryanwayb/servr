...
HttpServer.prototype.request = function(request, response) {
    ...
    if(this._rootPathMap.length > 1) {
        if(this._options.cache.urlMap && (cacheEntry = this._cache.urlMap[filterUrl]) !== undefined) {
...
HttpServer.prototype._responseWriteDirectory = function(rootPath, request, response, files, currentIndex) { // Takes a single path and an array of files and generates a response
    var cacheEntry;
    if(!currentIndex) {
        if(this._options.cache.directoryDefaults && (cacheEntry = this._cache.directoryDefaults[rootPath]) !== undefined) {
    ...
    var currentPath = files[currentIndex];

    if(this._options.cache.handlers && (cacheEntry = this._cache.handlers[currentPath])) {
        cacheEntry(request, response);
    }
    else if(this._options.cache.files && (cacheEntry = this._cache.files[currentPath])) {
        ...
        var self = this;
        fs.readFile(path.join(rootPath, currentPath), function(error, buffer) {
...
HttpServer.prototype._responseWriteDirectoryReadFileCalback = function(error, buffer, rootPath, request, response, files, currentIndex, currentPath) {
    if(!error) {
        ...
        if(mimetypes.EnumMimeTypeExt[fileExtension]) {
            ...
            if(cacheHandler && this._options.cache.handlers) {
                this._cache.handlers[currentPath] = cacheHandler;
            }
            else if(this._options.cache.files) {
            ...
            if(this._options.cache.files) {
                this._cache.files[currentPath] = buffer;
            }
        }

        if(this._options.cache.directoryDefaults) {
...
HttpServer.prototype._responseWriteFile = function(filepath, request, response) {
    var cacheEntry;
    if(this._options.cache.handlers && (cacheEntry = this._cache.handlers[filepath])) {
        cacheEntry(request, response);
    }
    else if(this._options.cache.files && (cacheEntry = this._cache.files[filepath])) {
