...
HttpServer.prototype.request = function(request, response) {
    ...
    if(this._rootPathMap.length > 1) {
        for(var current in this._rootPathMap) {
            if(this._rootPathMap.hasOwnProperty(current)) {
        ...
        if(!error) {
            if(stat.isDirectory()) {
                HttpServer.prototype._responseWriteDirectory.call(self, physicalPath,
                    self._config.directories[self._rootPathMap[urlMapIndex].url].default.slice(), response);
            }
            else if(stat.isFile()) {
...
HttpServer.prototype._responseWriteDirectory = function(rootPath, files, response) { // Takes a single path or an array of paths and generates a response
    if(files.length > 0) {
        var currentPath = files.splice(0, 1)[0];

        if(this._cache[currentPath]) {
            this._cache[currentPath](response);
        }
        else {
            var self = this;
            fs.readFile(path.join(rootPath, currentPath), function(error, buffer) {
                if(!error) {
                    ...
                    if(files.length > 0) {
                        HttpServer.prototype._responseWriteDirectory.call(self, rootPath, files, response);
                    }
                    else {
