...
function HttpServer(instance, config, useHttps) {
	...
	this._cacheEnabled = true; // TODO: Setup as well. Use true to mean everything is cache enabled, and use { '.fileExtension': true, ... } for specific file types
	this._cache = { };
}
	...
	for(var i in files) {
		(function(currentPath) {
			if(self._cache[currentPath]) {
				// No need to check mime types here, if it was added to the cache it should already have a mime type. This will need to be changed if caching of non-handled files is allowed.
